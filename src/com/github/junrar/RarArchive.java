 ...
 */
public class RarArchive implements Closeable {

  private static Logger logger = Logger.getLogger(RarArchive.class.getName());
  ...
  public RarArchive(File firstVolume) throws RarException, IOException {
    this(new FileVolumeManager(firstVolume), null);
  }

  public RarArchive(File firstVolume, UnrarCallback unrarCallback)
      throws RarException, IOException {
    this(new FileVolumeManager(firstVolume), unrarCallback);
  }

  public RarArchive(VolumeManager volumeManager) throws RarException,
      IOException {
   ...
   * @throws RarException
   */
  public RarArchive(VolumeManager volumeManager, UnrarCallback unrarCallback)
      throws RarException, IOException {
  ...
  private void readHeaders(long fileLength) throws IOException, RarException {
    ...
    while (true) {
      ...
      case MainHeader:
        ...
        byte[] mainbuff = safelyAllocate(toRead, MAX_HEADER_SIZE);//byte[] mainbuff = new byte[toRead];
      ...
      case EndArcHeader:
        ...
        if (toRead > 0) {
          byte[] endArchBuff = safelyAllocate(toRead, MAX_HEADER_SIZE);//byte[] endArchBuff = new byte[toRead];
      ...
      default:
        ...
        case SubHeader:
        {
          ...
          case MAC_HEAD:
          {
            byte[] macHeaderbuffer = safelyAllocate(MacInfoHeader.MacInfoHeaderSize, MAX_HEADER_SIZE);//byte[] macHeaderbuffer = new byte[MacInfoHeader.MacInfoHeaderSize];
          ...
          case EA_HEAD:
          {
            byte[] eaHeaderBuffer = safelyAllocate(EAHeader.EAHeaderSize, MAX_HEADER_SIZE);//byte[] eaHeaderBuffer = new byte[EAHeader.EAHeaderSize];
          ...
          case UO_HEAD:
            ...
            byte[] uoHeaderBuffer = safelyAllocate(toRead, MAX_HEADER_SIZE);//byte[] uoHeaderBuffer = new byte[toRead];

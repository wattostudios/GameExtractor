...
public class Resource_Unreal extends Resource {
  ...
  UnrealProperty[] unrealProperties = null;
  ...
  **/
  ...
  /**
  **********************************************************************************************
  Returns the Properties as well as the UnrealProperties
  **********************************************************************************************
  **/
  public Resource_Property[] getProperties() {
    Resource_Property[] properties = super.getProperties();

    if (unrealProperties != null && unrealProperties.length > 0) {
      int propertyLength = 0;
      if (properties != null) {
        propertyLength = properties.length;
      }
      int unrealLength = unrealProperties.length;

      int totalLength = propertyLength + unrealLength;

      Resource_Property[] totalProperties = new Resource_Property[totalLength];
      if (propertyLength != 0) {
        System.arraycopy(properties, 0, totalProperties, 0, propertyLength);
      }
      for (int i = propertyLength, j = 0; i < totalLength; i++, j++) {
        UnrealProperty unreal = unrealProperties[j];
        totalProperties[i] = new Resource_Property(unreal.getName(), unreal.getValue().toString());
      }
      ...
      // just return the normal properties
      return properties;
    }
  }

...
public class Exporter_Custom_VAG_Audio_Interleaved extends ExporterPlugin {

  static Exporter_Custom_VAG_Audio_Interleaved instance = new Exporter_Custom_VAG_Audio_Interleaved();
  ...
  **********************************************************************************************
  **/
  public static Exporter_Custom_VAG_Audio_Interleaved getInstance() {
    return instance;
  }
  ...
  **********************************************************************************************
  **/
  public Exporter_Custom_VAG_Audio_Interleaved() {
  ...
  **********************************************************************************************
  **/
  public Exporter_Custom_VAG_Audio_Interleaved(boolean writeHeadersIn) {
    super();
  ...
  **/
  ...
  public void open(Resource source) {
    try {
      ...
      // change from 12288-interleave to 16-interleave
      String blockSizeString = source.getProperty("InterleaveBlockSize");
      int blockSize = 12288;
      try {
      ...
      int halfSize = ((rawLength / blockSize) / 2) * blockSize;

      byte[] leftBytes = new byte[halfSize];
      byte[] rightBytes = new byte[halfSize];
      int leftPos = 0;
      int rightPos = 0;

      for (int i = 0; i < rawLength; i += (blockSize * 2)) {
        System.arraycopy(compBytes, i, leftBytes, leftPos, blockSize);
        System.arraycopy(compBytes, i + blockSize, rightBytes, rightPos, blockSize);

        leftPos += blockSize;
        rightPos += blockSize;
      }

      /*// not sure how to interleave it back to stereo right at the moment
      int outPos = 0;
      for (int i = 0; i < halfSize; i += 16) {
        System.arraycopy(leftBytes, i, compBytes, outPos, 16);
        System.arraycopy(rightBytes, i, compBytes, outPos + 16, 16);
        outPos += 32;
      }
      */
      // so just return mono instead
      compBytes = leftBytes;
      decompLength /= 2;

...
public class Exporter_ZLib_XXTEA extends ExporterPlugin {

  static Exporter_ZLib_XXTEA instance = new Exporter_ZLib_XXTEA();
  ...
  **********************************************************************************************
  **/
  public static Exporter_ZLib_XXTEA getInstance() {
    return instance;
  }
  ...
  **********************************************************************************************
  **/
  public Exporter_ZLib_XXTEA() {
    setName("ZLib Compression with XXTEA Encryption");
  }
  ...
  **********************************************************************************************
  **/
  public Exporter_ZLib_XXTEA(byte[] key) {
    setName("ZLib Compression with XXTEA Encryption");
    this.key = key;
  }
  ...
  **/
  public void open(FileManipulator fmIn, int compLengthIn, int decompLengthIn) {
    try {
      ...
      // read in the data
      byte[] buffer = fmIn.readBytes(compLengthIn); // compressed length
      // decrypt it
      buffer = XXTEA.decrypt(buffer, key);

      // open the decrypted data
      fm = new FileManipulator(new ByteBuffer(buffer));
  ...
  **/
  ...
  public void open(Resource source) {
    try {
      ...
      // read in the data
      byte[] buffer = fm.readBytes((int) source.getLength()); // compressed length
      // decrypt it
      buffer = XXTEA.decrypt(buffer, key);

...
**********************************************************************************************
**/
public class Plugin_UOO_2 extends ArchivePlugin {
  ...
  **/
  public Plugin_UOO_2() {
    ...
    setExtensions("uoo"); // MUST BE LOWER CASE
    setPlatforms("PC");

    // MUST BE LOWER CASE !!!
    setFileTypes(new FileType("uoo_anim", "Animated Texture", FileType.TYPE_IMAGE));
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      // Header (109,171,127,30)
      if (fm.readInt() == 511683437) {
        rating += 49; // one less than usual, so it tries the UOO plugin first
      }
  ...
  public Resource[] read(File path) {
    try {
      ...
      while (fm.getOffset() < arcSize - 16) {
        ...
        //System.out.println("Folder at " + (fm.getOffset() - 12) + " with " + numGroups + " groups");
        ...
        for (int g = 0; g < numGroups; g++) {
          ...
          FieldValidator.checkNumFiles(numFilesInGroup + 1); // allow zero files in the group

          //System.out.println("    Group at " + (fm.getOffset() - 8) + " with " + numFilesInGroup + " numFiles");

          for (int f = 0; f < numFilesInGroup; f++) {
            // 4 - File ID?
            ...
            //System.out.println("        File at " + (fm.getOffset() - 12) + " with " + length + " length");

            // X - File Data (Deflate Compression)
            long offset = fm.getOffset();
            fm.skip(length);

            String filename = "Folder " + folderID + "\\Group " + groupID + "\\" + Resource.generateFilename(fileID) + ".uoo_anim";

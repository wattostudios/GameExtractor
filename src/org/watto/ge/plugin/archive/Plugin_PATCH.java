...
**********************************************************************************************
**/
public class Plugin_PATCH extends ArchivePlugin {
  ...
  **/
  public Plugin_PATCH() {
    ...
    setExtensions("patch"); // MUST BE LOWER CASE
    setPlatforms("PS3");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (IntConverter.changeFormat(fm.readInt()) + 28 == fm.getLength()) {
        rating += 5;
      }
  ...
  public Resource[] read(File path) {
    try {
      ...
      while (fm.getOffset() < arcSize) {
        ...
        // 1 - Unknown (0=new file, 128=part of the previous file)
        // 1 - File Type (0=Unknown, 22=Texture, 88=Animation, 119=Material, 131=Model, ...)
        ...
        if (blockType != 0) {
          ...
          if (fileType < 0) {
            ...
            long totalLength = previousResource.getDecompressedLength() + length;

            long[] blockOffsets = new long[] { previousResource.getOffset(), offset };
            long[] blockLengths = new long[] { previousResource.getLength(), length };

            BlockExporterWrapper blockExporter = new BlockExporterWrapper(exporter, blockOffsets, blockLengths, blockLengths);

            previousResource.setLength(totalLength);
            previousResource.setDecompressedLength(totalLength);
            previousResource.setExporter(blockExporter);
        ...
        int padding = calculatePadding(blockLength, 16);
        fm.skip(padding);

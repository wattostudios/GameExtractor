...
**********************************************************************************************
**/
public class Plugin_GCF extends ArchivePlugin {
  ...
  **/
  ...
  public Resource[] read(File path) {
    try {
      ...
      // BLOCK ENTRY USAGE MAP (version 5 and earlier)

      if (version <= 5) {
        // 4 - Number Of Data Blocks
        // 4 - First Block Entry Index
        // 4 - Last Block Entry Index
        // 4 - Unknown
        // 4 - Checksum
      ...
      for (int i = 0; i < numEntries; i++) {
        ...
        if (entryType == 0) {
          ...
          int numFileBlocks = 0;
          long[] lengths = new long[arraySize];
          long[] offsets = new long[arraySize];
          ...
          if (blockIndex == -1) {
            ...
            offsets[0] = (firstDataBlockIndexes[blockIndex] * paddingMultiple) + relOffset;
            lengths[0] = blockLengths[blockIndex];
            numFileBlocks++;

            /*
             * if (realNumFiles == numFiles-1){ System.out.println("blockIndex " + blockIndex);
             * System.out.println("firstDataBlockIndexes " + firstDataBlockIndexes[blockIndex]);
             * System.out.println("blockLengths " + blockLengths[blockIndex]);
             * System.out.println("nextDataBlockIndexes " + nextDataBlockIndexes[blockIndex]);
             * System.out.println("================"); }
             */

            blockIndex = nextDataBlockIndexes[blockIndex];
          }

          while (blockIndex != -1) {
            if (numFileBlocks == offsets.length) {
              // expand the arrays
              long[] temp = lengths;
              lengths = new long[numFileBlocks + 10];
              System.arraycopy(temp, 0, lengths, 0, numFileBlocks);

              temp = offsets;
              offsets = new long[numFileBlocks + 10];
              System.arraycopy(temp, 0, offsets, 0, numFileBlocks);
            }

            offsets[numFileBlocks] = (firstDataBlockIndexes[blockIndex] * paddingMultiple) + relOffset;
            lengths[numFileBlocks] = blockLengths[blockIndex];
            numFileBlocks++;
          ...
          if (numFileBlocks < offsets.length) {
            // shrink the arrays to the correct size
            long[] temp = lengths;
            lengths = new long[numFileBlocks];
            System.arraycopy(temp, 0, lengths, 0, numFileBlocks);

            temp = offsets;
            offsets = new long[numFileBlocks];
            System.arraycopy(temp, 0, offsets, 0, numFileBlocks);
          }

          //path,name,offset,length,decompLength,exporter
          resources[realNumFiles] = new SplitChunkResource(path, names[i], offsets, lengths);
          realNumFiles++;
        }

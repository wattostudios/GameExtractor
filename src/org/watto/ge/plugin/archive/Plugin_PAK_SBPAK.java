...
public class Plugin_PAK_SBPAK extends ArchivePlugin {
  ...
  **/
  public Plugin_PAK_SBPAK() {
    ...
    setTextPreviewExtensions("csv", "fx", "inc", "shaderlib", "vexporter"); // LOWER CASE
    ...
    try {
      ...
      */
      ...
      for (int i = 0; i < numFiles; i++) {
        ...
        // X - Filename
        int maxLength = (int) nameFM.getRemainingLength(); // set an upper limit
        String filename = null;
        if (maxLength < 0) {
          filename = Resource.generateFilename(i);
        }
        else {
          filename = nameFM.readNullString(maxLength);

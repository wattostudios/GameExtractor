...
**********************************************************************************************
**/
public class Plugin_UE3_807 extends PluginGroup_UE3 {
  ...
  **/
  ...
  public Resource[] read(File path) {
    try {
      ...
      */
      ...
      /*
      // now go through and read the Unreal properties for each file
      fm.getBuffer().setBufferSize(256);
      for (int i = 0; i < numFiles; i++) {
        Resource_Unreal resource = resources[i];
      
        if (resource.getLength() < 14) {
          continue; // file is too small
        }
        ...
        if (properties != null && properties.length > 0) {
          ...
          int propertiesLength = (int) (endOffset - startOffset);
          int length = (int) (resource.getLength() - propertiesLength);
      
          if (length > 0) {
            resource.setUnrealProperties(properties);
   ...
   * THIS IS DIFFERENT - this has a separate 2-byte field at the beginning indicating whether
   * there are 2 fields or more in the entry. This removes the need for the ClassID property taking
   * up 8 bytes, provided the classID matches to something basic (like an Int or something)
   **********************************************************************************************
   **/
  public UnrealProperty readProperty(FileManipulator fm) {
    try {
      ...
      // 2 - Field Length
      short fieldLength = fm.readShort();
      if (fieldLength == 0) {
        return null;
      }
      /*
      ...
      */
      ...
      if (length == 4) {
        ...
        property.setValue(names[(int) longValue]);
      }

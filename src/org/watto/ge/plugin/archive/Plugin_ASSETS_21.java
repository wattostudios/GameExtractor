...
**********************************************************************************************
**/
public class Plugin_ASSETS_21 extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_ASSETS_21() {

    super("ASSETS_21", "Unity3D Engine Resource (Version 21)");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("Coffee Noir: Business Detective Game");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      // 4 - Version (21) (BIG ENDIAN)
      if (IntConverter.changeFormat(fm.readInt()) == 21) {
        rating += 5;
      }
    ...
    try {
      ...
      if (header.substring(0, 4).equals("FSB5")) {
        ...
        if ((flags & 3) == 3) {
          ...
          if (decompWritePos == 0) {
            // Failed to decompress the header, move along a little bit and try again
            fm.relativeSeek(64);
          ...
          // LZMA Compression
          ErrorLogger.log("[Plugin_ASSETS_21] LZMA Compression Not Implemented");
          return null;
        }
        else {
          // no compression
      ...
      if (version == 22) {
        // We can get here is we decompress a unity3d file, and are now trying to read each individual file in it, but the
        // individual files are actually v22 instead of v21
        return new Plugin_ASSETS_22().read(path);
      }

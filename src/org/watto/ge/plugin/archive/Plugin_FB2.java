...
**********************************************************************************************
**/
public class Plugin_FB2 extends ArchivePlugin {
  ...
  **/
  public Plugin_FB2() {
    ...
    setGames("Battlefield 3");
    setExtensions("fb2"); // MUST BE LOWER CASE
    setPlatforms("PC");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      // Header
      if (fm.readInt() == 13553920) {
        rating += 24; // don't want to match non-*.fb2 files
      }
   ...
   **********************************************************************************************
   **/
  public File decryptArchive(FileManipulator fm) {
    try {
      ...
      for (int i = 0; i < remainingLength; i++) {
        decompFM.writeByte((byte) (ByteConverter.unsign(fm.readByte()) ^ key[i % 257]));
      }
  ...
  public Resource[] read(File path) {
    try {
      ...
      if (header == 13553920 || header == 30331136) {
        ...
        File decompFile = decryptArchive(fm);
        if (decompFile != null && decompFile.exists()) {

          Resource[] resources = new Plugin_ZIP_PK().read(decompFile);

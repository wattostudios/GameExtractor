...
**********************************************************************************************
**/
public class Plugin_IPK extends ArchivePlugin {
  ...
  **/
  public Plugin_IPK() {
    ...
    setGames("Rayman Legends");
    setExtensions("ipk"); // MUST BE LOWER CASE
    setPlatforms("PC");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      // Header?
      if (IntConverter.changeFormat(fm.readInt()) == 1357648570) {
        rating += 25;
      }
  ...
  public Resource[] read(File path) {
    try {
      ...
      // 8 - File Data Offset
      long fileDataOffset = LongConverter.convertBig(fm.readBytes(8));
      ...
      for (int i = 0; i < numFiles; i++) {
        ...
        // 8 - File Offset
        long offset = LongConverter.convertBig(fm.readBytes(8)) + fileDataOffset;
        FieldValidator.checkOffset(offset, arcSize);

        // 4 - Directory Name Length
        int dirNameLength = IntConverter.changeFormat(fm.readInt());

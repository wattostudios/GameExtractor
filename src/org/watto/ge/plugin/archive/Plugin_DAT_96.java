...
import org.watto.ge.plugin.viewer.Viewer_DAT_96_DATTEX;
...
**********************************************************************************************
**/
public class Plugin_DAT_96 extends ArchivePlugin {
  ...
  **/
  public Plugin_DAT_96() {
    ...
    setFileTypes(new FileType("dat_tex", "TEX Image", FileType.TYPE_IMAGE));
  ...
  **/
  ...
  public Resource[] read(File path) {
    try {
      ...
      fm.skip(calculatePadding(description.length() + 1, 4));

      // 4 - Number of Images [/2]
      FieldValidator.checkNumFiles(fm.readInt());

      int numFiles = (int) (fm.getLength() - fm.getOffset()) / 8;
  ...
  public File convertOnReplace(Resource resourceBeingReplaced, File fileToReplaceWith) {
    ...
    if (beingReplacedExtension.equalsIgnoreCase("dat_tex")) {
      // try to convert

      String toReplaceWithExtension = FilenameSplitter.getExtension(fileToReplaceWith);
      if (toReplaceWithExtension.equalsIgnoreCase("dat_tex")) {
      ...
      //
      //
      Viewer_DAT_96_DATTEX converterPlugin = new Viewer_DAT_96_DATTEX();

...
**********************************************************************************************
**/
public class Plugin_PAK_66 extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_PAK_66() {

    super("PAK_66", "Unreal Engine 4 PAK Archive with Encrypted Directory [PAK_66] ");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("Little Nightmares 2");
    ...
    setEnabled(false); // CURRENTLY DISABLED (it works, but because you don't have filenames, you can't find the matching uexp/ubulk files)
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (compressionType == 0 || compressionType == 1 || compressionType == 2 || compressionType == 4) {
        rating += 3; // 3, so that this plugin is only tried *after* the PAK_38 and PAK_65 plugins, which read most UE4 archives
      }
  ...
  public Resource[] read(File path) {
    try {
      ...
      FileManipulator fm = new FileManipulator(path, false, 64); // 64-byte buffer for quick reads
      ...
      int realNumFiles = 0;
      Resource_PAK_38 previousResource = null;
      while (fm.getOffset() < arcSize) {
        ...
        try {
          ...
          if (compressionType == 0) {
            ...
            if (length != 0) {
              Resource_PAK_38 resource = new Resource_PAK_38(path, filename, offset, length);
          ...
          // not sure, hopefully just some padding... skip it (and add it to the previous file)
          fm.skip(calculatePadding(fm.getOffset(), 2048));

          System.out.println("Adding 2048");

          if (previousResource != null) {
            long length = previousResource.getDecompressedLength() + 2048;
      ...
      // Now assume all are uasset files, try to process them for a class name

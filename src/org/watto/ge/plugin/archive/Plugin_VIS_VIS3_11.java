...
**********************************************************************************************
**/
public class Plugin_VIS_VIS3_11 extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_VIS_VIS3_11() {

    super("VIS_VIS3_11", "VIS_VIS3_11");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("The Night of the Rabbit");
  ...
  **/
  ...
  public Resource[] read(File path) {
    try {
      ...
      //101,55,99,56,51,101,52,50,50,100,57,50,52,102,56,98
      for (int i = 0; i < dirSize; i += 16) {
        directoryBytes[i] ^= 101;
        directoryBytes[i + 1] ^= 55;
        directoryBytes[i + 2] ^= 99;
        directoryBytes[i + 3] ^= 56;
        directoryBytes[i + 4] ^= 51;
        directoryBytes[i + 5] ^= 101;
        directoryBytes[i + 6] ^= 52;
        directoryBytes[i + 7] ^= 50;

        directoryBytes[i + 8] ^= 50;
        directoryBytes[i + 9] ^= 100;
        directoryBytes[i + 10] ^= 57;
        directoryBytes[i + 11] ^= 50;
        directoryBytes[i + 12] ^= 52;
        directoryBytes[i + 13] ^= 102;
        directoryBytes[i + 14] ^= 156;
        directoryBytes[i + 15] ^= 98;
      }

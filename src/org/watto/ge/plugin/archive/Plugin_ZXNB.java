...
**********************************************************************************************
**/
public class Plugin_ZXNB extends ArchivePlugin {
  ...
  **/
  public Plugin_ZXNB() {
    ...
    super("ZXNB", "Compressed Microsoft XNB Archive [ZXNB]");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("Mercenary Kings");
    setExtensions("zxnb"); // MUST BE LOWER CASE
    setPlatforms("PC");
   ...
   **********************************************************************************************
   **/
  public FileManipulator decompressArchiveLZX(FileManipulator fm, int compLength, int decompLength) {
    try {
      ...
      File decompFile = new File(pathOnly + File.separatorChar + filenameOnly + "_ge_decompressed_lzx" + "." + extensionOnly);
   ...
   **********************************************************************************************
   **/
  public FileManipulator decompressArchiveDeflate(FileManipulator fm, int compLength, int decompLength) {
    try {
      // Build a new "_ge_decompressed" archive file in the current directory
      File origFile = fm.getFile();
      ...
      File decompFile = new File(pathOnly + File.separatorChar + filenameOnly + ".xnb");
      if (decompFile.exists()) {
        // we've already decompressed this file before - open and return it
        return new FileManipulator(decompFile, false);
      }
  ...
  **/
  ...
  public Resource[] read(File path) {
    try {
      ...
      // X - Compressed Data (Deflate Compression)
      FileManipulator decompFM = decompressArchiveDeflate(fm, (int) arcSize, (int) arcSize);
      if (decompFM != null) {
      ...
      if ((flags & 128) == 128) {
        ...
        decompFM = decompressArchiveLZX(fm, compLength, decompLength);
        if (decompFM != null) {
          fm.close(); // close the original archive
          fm = decompFM; // now we're going to read from the decompressed file instead
          fm.seek(0); // go to the start of the file

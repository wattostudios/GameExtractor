...
**********************************************************************************************
**/
public class Plugin_CRS_MDMD extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_CRS_MDMD() {

    super("CRS_MDMD", "CRS_MDMD");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("The Original LINKS");
    setExtensions("crs", "lz"); // MUST BE LOWER CASE
    setPlatforms("PC");
  ...
  **/
  ...
  public Resource[] read(File path) {
    try {
      ...
      //ExporterPlugin exporter = new Exporter_QuickBMS_Decompression("ULTIMA6");
      ...
      while (fm.getOffset() < arcSize) {
        ...
        // 1 - Filename Length (not including padding)
        int filenameLength = fm.readByte();

        // 12 - Filename (padded to length 12 with (byte)32)
        String filename = fm.readString(filenameLength);
        fm.skip(12 - filenameLength);

        // 1 - Directory Name length (not including padding)
        int dirNameLength = fm.readByte();

        // 67 - Directory Name (padded to length 67 with (byte)32)
        filename = fm.readString(dirNameLength) + filename;
        fm.skip(67 - dirNameLength);

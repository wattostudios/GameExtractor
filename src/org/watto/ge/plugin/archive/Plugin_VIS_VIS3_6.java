...
**********************************************************************************************
**/
public class Plugin_VIS_VIS3_6 extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_VIS_VIS3_6() {

    super("VIS_VIS3_6", "VIS_VIS3_6");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("Chaos on Deponia (Humble Bundle)");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (fm.readByte() == 55 && fm.readByte() == 101 && fm.readByte() == 99 && fm.readByte() == 99) {
        rating += 5;
      }
      else {
        rating = 0;
  ...
  public Resource[] read(File path) {
    try {
      ...
      //52,49,97,55,101,99,99,56
      for (int i = 0; i < dirSize; i += 8) {
        directoryBytes[i] ^= 52;
        directoryBytes[i + 1] ^= 49;
        directoryBytes[i + 2] ^= 97;
        directoryBytes[i + 3] ^= 55;
        directoryBytes[i + 4] ^= 101;
        directoryBytes[i + 5] ^= 99;
        directoryBytes[i + 6] ^= 99;

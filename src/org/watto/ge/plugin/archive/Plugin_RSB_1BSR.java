...
**********************************************************************************************
**/
public class Plugin_RSB_1BSR extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_RSB_1BSR() {

    super("RSB_1BSR", "RSB_1BSR");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      // Header
      if (fm.readString(4).equals("1bsr")) {
        rating += 50;
      }
  ...
  public Resource[] read(File path) {
    try {
      ...
      for (int i = 0; i < numGroups; i++) {
        ...
        int groupDataLength = fm.readInt();
        ...
        while (fm.getOffset() < groupEndOffset) {
          ...
          for (int p = 0; p < 512; p++) { // 512 as a maximum filename length (guessed)
            ...
            if (bytes4[0] == 0) {
              ...
              filenameBytes[filenamePos] = bytes4[0];
              if (bytes4[1] != 0) {
                ...
                byte[] expireOffsetBytes = new byte[] { bytes4[1], bytes4[2], bytes4[3], 0 };
          ...
          if (compressed == 1) {
            ...
            if (decompLength == decompressedBlockLength) {
              ...
              if (filename.endsWith("_01.PTX")) {
                resource.addProperty("ImageFormat", "BARG4444");
              }
              else if (imageFormat == 1 || filename.contains("BOSSINTRO")) {
                resource.addProperty("ImageFormat", "RGB565");
              }
              else if (imageFormat == 0) {
                resource.addProperty("ImageFormat", "BARG4444");
              }
              else {
                ErrorLogger.log("[RSB_1BSR]: Unknown Image Format: " + imageFormat);
                resource.addProperty("ImageFormat", "BARG4444");
              }
              ...
              SubsetExporterWrapper exporterGroup = new SubsetExporterWrapper(exporter, relDataOffset, compressedBlockLength, decompressedBlockLength, relFileOffset, decompLength);

              //path,name,offset,length,decompLength,exporter
              Resource resource = new Resource(path, filename, relDataOffset, compressedBlockLength, decompressedBlockLength, exporterGroup);
              resource.addProperty("Width", "" + imageWidth);
              resource.addProperty("Height", "" + imageHeight);
              if (filename.endsWith("_01.PTX")) {
                resource.addProperty("ImageFormat", "BARG4444");
            ...
            // 4 - File Data Offset (relative to the start of the File Data for the files in this resource group)
            int offset = (int) (fm.readInt() + relDataOffset);
            FieldValidator.checkOffset(offset, arcSize);
            ...
            ErrorLogger.log("[RSB_1BSR]: Unknown compression: " + compressed);
          }

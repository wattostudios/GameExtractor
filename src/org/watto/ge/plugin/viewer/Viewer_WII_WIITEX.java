...
import org.watto.ge.plugin.archive.Plugin_WII;
...
**********************************************************************************************
**/
public class Viewer_WII_WIITEX extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_WII_WIITEX() {
    super("WII_WIITEX", "WII_WIITEX Image");
    setExtensions("wii_tex");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (plugin instanceof Plugin_WII) {
        rating += 50;
      }
      ...
      if (IntConverter.changeFormat(fm.readInt()) == 192) {
        rating += 5;
      }
  ...
  **/
  ...
  public ImageResource readThumbnail(FileManipulator fm) {
    try {
      ...
      // 2 - Unknown (2)
      // 4 - null
      // 1 - File Type (60=Text String, 34=Image, 99=Other File)
      // 3 - File ID?
      // 32 - Filename (null terminated, filled with nulls)
      // 16 - Padding (byte 239)
      // 32 - Filename without Extension (null terminated, filled with junk)
      // 4 - null
      // 4 - Number of Mipmaps
      fm.skip(104);

      // 4 - Header Length (192)
      int imageDataOffset = IntConverter.changeFormat(fm.readInt());
      ...
      // 4 - Image Format (5=RGB5A3Wii, 6=ARGB+Swizzle, 14=CMPR)
      // NOTE: Seems to match with TPL image format codes - see https://wiki.tockdom.com/wiki/TPL_%28File_Format%29
      int imageFormat = IntConverter.changeFormat(fm.readInt());

      // 4 - null
      // 28 - Padding (byte 239) to a multiple of 32 bytes
      ...
      if (imageFormat == 5) {
        // RGB5A3Wii
        imageResource = ImageFormatReader.readRGB5A3Wii(fm, width, height);
      }
        ...
        paddedWidth += ArchivePlugin.calculatePadding(paddedWidth, 4);
        paddedHeight += ArchivePlugin.calculatePadding(paddedHeight, 4);

        int numBytes = paddedWidth * paddedHeight * 4;

        byte[] pixelBytes = fm.readBytes(numBytes);
        pixelBytes = ImageSwizzler.unswizzleGameCube32Bit(pixelBytes, paddedWidth, paddedHeight);

        fm.close();
        fm = new FileManipulator(new ByteBuffer(pixelBytes));

        imageResource = ImageFormatReader.readARGB(fm, paddedWidth, paddedHeight);
      }
        ...
        ErrorLogger.log("[Viewer_WII_WIITEX] Unknown Image Format: " + imageFormat);
        return null;
      }

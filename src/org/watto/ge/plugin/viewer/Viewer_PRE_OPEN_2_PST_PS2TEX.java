...
import org.watto.ge.plugin.archive.Plugin_PRE_OPEN_2;
...
**********************************************************************************************
**/
public class Viewer_PRE_OPEN_2_PST_PS2TEX extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_PRE_OPEN_2_PST_PS2TEX() {
    super("PRE_OPEN_2_PST_PS2TEX", "PS2Tex V0.6 Image");
    setExtensions("pst");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (plugin instanceof Plugin_PRE_OPEN_2) {
        rating += 50;
      }
      ...
      // 11 - Header
      if (fm.readString(11).equals("PS2Tex V0.6")) {
        rating += 50;
      }
      else {
        rating = 0;
  ...
  **/
  ...
  public ImageResource readThumbnail(FileManipulator fm) {
    try {
      ...
      // 12 - Header ("PS2Tex V0.6" + null);
      fm.skip(12);

      // 4 - Image Type (3=8bit Paletted, 4=4bit Paletted)
      ...
      if (imageFormat == 3) {
        ...
        palette = ImageSwizzler.unstripePalettePS2(palette); // unstripe
        palette = ImageFormatReader.doubleAlpha(palette); // double the alpha

        // X - Pixels
        int numPixels = width * height;
        byte[] pixelBytes = fm.readBytes(numPixels);
        pixelBytes = ImageSwizzler.unswizzlePS28BitSuba(pixelBytes, width, height); //unswizzle 
        ...
        //palette = ImageSwizzler.unstripePalettePS2(palette); // unstripe NOT FOR 4-BIT
        palette = ImageFormatReader.doubleAlpha(palette); // double the alpha
        ...
        for (int i = 0; i < numPixels; i += 2) {
          ...
          pixelBytes[i] = (byte) pixel1;
          pixelBytes[i + 1] = (byte) pixel2;
        }
        ...
        ErrorLogger.log("[Viewer_PRE_OPEN_2_PST_PS2TEX] Unknown Image Format: " + imageFormat);
        return null;
      }

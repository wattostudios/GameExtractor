...
**********************************************************************************************
**/
public class Viewer_SDU_SRSC_SDUMDL extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_SDU_SRSC_SDUMDL() {
    super("SDU_SRSC_SDUMDL", "The Suffering SDU_MDL Model");
    setExtensions("sdu_mdl");

    setGames("The Suffering");
    setPlatforms("PC");
    setStandardFileFormat(false);

    setEnabled(false); // not currently working - can't align face indexes with the points
  }
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (readPlugin instanceof Plugin_SDU_SRSC) {
        rating += 50;
      }
  ...
  **/
  ...
  public PreviewPanel read(FileManipulator fm) {
    ...
    try {
      ...
      // 4 - Number of Faces / Number of Vertices
      ...
      for (int i = 0, j = 0; i < numFaces; i++, j += 6) {
        // 4 - Number of Indexes in this face (3)
        int numIndexes = fm.readInt();
        if (numIndexes != 3) {
          ErrorLogger.log("[Viewer_SDU_SRSC_SDUMDL] Invalid number of indexes in this face: " + numIndexes);
        }

        // 2 - Face Index 1 (Triangle Point 1)
        // 2 - Face Index 2 (Triangle Point 2)
        // 2 - Face Index 3 (Triangle Point 3)
        ...
        if (facePoint1 > maxFaceIndex) {
          maxFaceIndex = facePoint1;
        }
        if (facePoint2 > maxFaceIndex) {
          maxFaceIndex = facePoint2;
        }
        if (facePoint3 > maxFaceIndex) {
          maxFaceIndex = facePoint3;
        }

        // skip any additional indexes, in case there's more than 3
        fm.skip((numIndexes - 3) * 2);

        // 0-2 - null Padding to a multiple of 4 bytes
        fm.skip(ArchivePlugin.calculatePadding(numIndexes * 2, 4));

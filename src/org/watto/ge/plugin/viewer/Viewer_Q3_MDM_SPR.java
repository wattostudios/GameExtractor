...
import org.watto.ge.plugin.archive.Plugin_Q3_MDM;
...
**********************************************************************************************
**/
public class Viewer_Q3_MDM_SPR extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_Q3_MDM_SPR() {
    super("Q3_MDM_SPR", "Q3_MDM_SPR Image");
    setExtensions("spr");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (plugin instanceof Plugin_Q3_MDM) {
        rating += 50;
      }
  ...
  **/
  ...
  public ImageResource readThumbnail(FileManipulator fm) {
    try {
      ...
      // first, get all the image data to a buffer, so the thumbnail renders correctly
      int numPixels = width * height;
      byte[] imageBytes = fm.readBytes(numPixels);

      // load all the color palettes, if needed
      if (PaletteManager.getNumPalettes() <= 0) {
        ...
        for (int i = 0; i < numResources; i++) {
          ...
          if (resource.getExtension().equals("pal")) {
            // found a color palette

            ByteBuffer buffer = new ByteBuffer((int) resource.getLength());
            ...
            int[] palette = ImageFormatReader.readRGB(palFM, 1, 256).getImagePixels();
      ...
      // now load the image from the buffer
      fm.close();

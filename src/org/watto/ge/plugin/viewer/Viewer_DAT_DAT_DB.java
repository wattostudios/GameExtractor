...
**********************************************************************************************
**/
public class Viewer_DAT_DAT_DB extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_DAT_DAT_DB() {
    super("DAT_DAT_DB", "ESPN NHL Hockey DB Table");
    setExtensions("db");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (filename.equals("feplayer.db") || filename.equals("feplayer_aux.db") || filename.equals("trophywinners.db")) {
        rating += 5;
      }
      else {
        rating = 0;
  ...
  **/
  ...
  public PreviewPanel read(FileManipulator fm) {
    try {
      ...
      if (filename.equals("feplayer.db")) {
      ...
      else if (filename.equals("trophywinners.db")) {
        int lineSize = 52;
        ...
        columns[0] = new WSTableColumn("Original Year", 'y', Integer.class, false, true, 0, 0); // hidden column 0,0
        columns[1] = new WSTableColumn("Edited Year", 'z', Integer.class, true, true);
        columns[2] = new WSTableColumn("Original Name", 'o', String.class, false, true, 0, 0); // hidden column 0,0
        columns[3] = new WSTableColumn("Edited Name", 'e', String.class, true, true);
        ...
        for (int k = 0; k < numKeys; k++) {
          ...
          // 48 - Text
          String text = fm.readNullString(48);

          data[k] = new Object[] { year, year, text, text };
        }
    ...
    try {
      ...
      if (filename.equals("feplayer.db")) {
        ...
        for (int k = 0; k < numRows; k++) {
          ...
          // 4 - Year
          int year = (Integer) currentRow[1];
          fm.writeInt(year);

          // 48 - Text
          String text = ((String) currentRow[3]);
          if (text.length() > 48) {
            text = text.substring(0, 48);
          }
          int textRemaining = 48 - text.length();

...
**********************************************************************************************
**/
public class Viewer_UE3_Texture2D_Generic extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_UE3_Texture2D_Generic() {
    super("UE3_Texture2D_Generic", "Unreal Engine v3/v4 Texture Image");
      ...
      for (int p = 0; p < numProperties; p++) {
        ...
        if (name.equals("Format")) {
          ...
          try {
            ...
            if (formatObject instanceof Long) {
              formatNameID = ((Long) formatObject).longValue();
            }
            else if (formatObject instanceof Integer) {
              formatNameID = ((Integer) formatObject).longValue();
            }
            else if (formatObject instanceof Short) {
              formatNameID = ((Short) formatObject).longValue();
            }
            else if (formatObject instanceof Byte) {
              formatNameID = ((Byte) formatObject).longValue();
            }
          }
          catch (Throwable t) {
          }
          ...
          if (formatName != null) {
            if (formatName.equals("PF_DXT5")) {
              ...
              // Earlier versions reference an ImageFormat ID directly, rather than by nameID
              if (formatNameID == 2) {
                imageFormat = "BGRA";
              }
              else if (formatNameID == 3) {
                imageFormat = "G8";
              }
              else if (formatNameID == 5) {
                imageFormat = "DXT1";
              }
              else if (formatNameID == 7) {
                imageFormat = "DXT5";
              }
              else {
                ErrorLogger.log("[Viewer_UE3_Texture2D_Generic] Unsupported Image Format: " + formatName + " (" + formatNameID + ")");
                return null;
              }
            }
          }

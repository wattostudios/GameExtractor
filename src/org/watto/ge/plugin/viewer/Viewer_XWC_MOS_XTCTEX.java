...
import org.watto.ge.plugin.archive.Plugin_XWC_MOS;
...
**********************************************************************************************
**/
public class Viewer_XWC_MOS_XTCTEX extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_XWC_MOS_XTCTEX() {
    super("XWC_MOS_XTCTEX", "XTC_TEX Image");
    setExtensions("xtc_tex");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (plugin instanceof Plugin_XWC_MOS) {
        rating += 50;
      }
  ...
  **/
  ...
  public ImageResource readThumbnail(FileManipulator fm) {
    try {
      ...
      // for each mipmap
      //   4 - Mipmap Offset (absolute, position in the archive)
      //fm.skip(mipmapCount * 4);
      ...
      if (imageFormat == 2048) {
        ...
        //   4 - Image Format? (0=DXT1, 4=DXT5)
        imageFormat = fm.readInt();

        if (imageLength == width * height * 4) {
          imageFormat = 64;
          removeAlpha = true;
          fm.relativeSeek(offset);
        }
        else if (imageFormat != 0 && imageFormat != 2 && imageFormat != 4) {
          imageFormat = 0;
          fm.relativeSeek(offset);
        }
        else {
      ...
      else if (imageFormat == 8192 || imageFormat == 262144 || imageFormat == 2) {
        imageResource = ImageFormatReader.read8BitPaletted(fm, width, height);
      }
      else {
        ErrorLogger.log("[Viewer_XWC_MOS_XTCTEX] Unknown Image Format: " + imageFormat);
        return null;
      }

...
import org.watto.ge.plugin.archive.Plugin_UE3_539;
...
**********************************************************************************************
**/
public class Viewer_UE3_Texture2D_539 extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_UE3_Texture2D_539() {
    super("UE3_Texture2D_539", "Unreal Engine 3 Version 539 Texture Image");
    setExtensions("texture2d"); // MUST BE LOWER CASE

    setGames("Unreal Engine 3 Version 539",
        "Alpha Protocol");
    setPlatforms("PC");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (readPlugin instanceof Plugin_UE3_539) {
        rating += 50;
      }
  ...
  public ImageResource readThumbnail(FileManipulator fm) {
    try {
      ...
      for (int m = 0; m < numMipmaps; m++) {
        ...
        if (extDecompLength == 0 && extCompLength == -1) {
          // not sure where this file is - not stored here
        }
        else {
        ...
        // 4 - Image Width
        imageWidth = fm.readInt() / 2; // /2, as this image didn't exist in the archive, so we actually want to set the width/height to the next mipmap size, not this size
        FieldValidator.checkWidth(imageWidth);

        // 4 - Image Height
        imageHeight = fm.readInt() / 2; // /2, as this image didn't exist in the archive, so we actually want to set the width/height to the next mipmap size, not this size
      ...
      if (fm.readByte() == -63 && fm.readByte() == -125 && fm.readByte() == 42 && fm.readByte() == -98) {
        ...
        if (compressionType != 1 && compressionType != 2) {
          ...
          ErrorLogger.log("[Viewer_UE3_Texture2D_539] Unsupported compression: " + compressionType);
          return null;
        }
        ...
        for (int d = 0; d < realNumBlocks; d++) {
          ...
          // make sure the file pointer is at the right spot, ready for the next block to be decompressed
          fm.seek(startOffset + compBlock);
        }

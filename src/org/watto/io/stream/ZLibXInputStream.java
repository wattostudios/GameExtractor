...
/***********************************************************************************************
 * ZLibX decompression Ported from C-code at https://bitbucket.org/jibsen/tinf
 * and modified to suit ZLibX format
 ***********************************************************************************************/
public class ZLibXInputStream extends InputStream {

  /** special ordering of code length codes **/
  short clcidx[] = { 16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15 };
  ...
  boolean continueUntilDecompLength = false;

  public boolean isContinueUntilDecompLength() {
    return continueUntilDecompLength;
  }

  public void setContinueUntilDecompLength(boolean continueUntilDecompLength) {
    this.continueUntilDecompLength = continueUntilDecompLength;
  }
  ...
  public ZLibXInputStream(File file, long decompLength) {
    ...
    // this.destManipulator = new FileManipulator(new ByteBuffer((int)
    // decompLength)); // moved to available();
    ...
    /*
     * while (destManipulator.getOffset() < decompLength) { // Skip the first 2
     * bytes, as per reference in http://aluigi.altervista.org/papers/quickbms.txt
     * // These 2 bytes are the data length for this compressed block
     * sourceManipulator.skip(2);
     * 
     * // Uncompress the block tinf_uncompress(); }
     * 
     * // Now that we've uncompressed all the data, we need to return the buffer to
     * the beginning, // ready for the read() by the extract() method when trying to
     * export this file to disk destManipulator.seek(0);
     */
  }
  ...
  public ZLibXInputStream(Manipulator manipulator, long decompLength) {
     ...
     * sourceManipulator.getOffset()); // Skip the first 2 bytes, as per reference
     * in http://aluigi.altervista.org/papers/quickbms.txt // These 2 bytes are the
     * data length for this compressed block int blockLength =
     * ShortConverter.unsign(sourceManipulator.readShort());
     * System.out.println(">>> Block length = " + blockLength +
     * ", next block should start at " + (sourceManipulator.getOffset() +
     * blockLength));
     * 
     * // Uncompress the block tinf_uncompress(); }
     * 
     * System.out.println(">>> Done. Read offset = " +
  ...
  /***********************************************************************************************
   * Gets the number of bytes left to read from the <code>Manipulator</code>
   * source
   * 
    ...
    if ((int) destManipulator.getRemainingLength() <= 0) {
      if (decompLength > 0) {
        ...
        while (sourceManipulator.getOffset() < nextBlockOffset) {
          // System.out.println(">>> Decompressing at offset " +
          // sourceManipulator.getOffset());
          ...
          if (destManipulator.getBuffer().getBufferLevel() >= decompLength) {
            // we've already decompressed enough data, so return;
            nextBlockOffset = 0;
          }

          resetBitCounter = false; // so remaining iterations through the while() will continue from the existing
                                   // bit, not reset it
        }
   ...
   * 
   * @param byteArray the array to read the <code>byte</code>s in to
   * @param offset    the offset in the <code>byteArray</code> to store the read
   *                  values
  ...
  /***********************************************************************************************
   * Moves to the <code>mark</code>ed position in the <code>Manipulator</code>
   * source, and resets the <code>mark</code>ed position
   ***********************************************************************************************/
  @Override
  public void reset() {
    try {
  ...
  /***********************************************************************************************
   * build extra bits and base tables bits = unsigned char base = unsigned short
   ***********************************************************************************************/
  ...
  /***********************************************************************************************
   * given an array of code lengths, build a tree lengths = const unsigned char
   * num = unsigned int
   ***********************************************************************************************/
  ...
  public void tinf_decode_trees(ZLibXTree lt, ZLibXTree dt) {
    ...
    // System.out.println(" tinf_decode_trees: Decoding Tree: hlit = " + hlit + ",
    // hdist = " + hdist + ", hclen = " + hclen);
  ...
  public int tinf_inflate_block_data(ZLibXTree lt, ZLibXTree dt) {
    while (true) {
      ...
      if (sym < 256) {
        ...
        // System.out.println(" tinf_inflate_block_data: Copy from previous: Length = "
        // + length + ", Dist = " + dist + ", Offset = " + offs);

        // copy match
        for (int i = 0; i < length; ++i) {
          // d->dest[i] = d->dest[i - offs];
          long existingOffset = destManipulator.getOffset();
          //System.out.println(existingOffset);
  ...
  public int tinf_inflate_uncompressed_block() {
    ...
    // System.out.println(" tinf_inflate_uncompressed_block: Inverse Length = " +
    // invlength);

    // check length
    if (length != (~invlength & 0x0000ffff)) {
      // System.out.println(" ERROR >> tinf_inflate_uncompressed_block: length and
      // invlength don't match");
      return -3;
    }

    // copy block (do it individually, not en masse, so we can duplicate parts
    // written by earlier iterations of this for loop)
  ...
  /***********************************************************************************************
   * Reads a number of bits from the source, and returns the value. Adds the base
   * value if provided
   ***********************************************************************************************/
  ...
  public int tinf_uncompress(boolean resetBitCounter) {
    ...
    // ZLIBX differs from ZLIB in that ZLib continues looping over and over, whereas
    // ZLIBX only reads the single
    // block before returning. Each block is preceeded by the length of the
    // compressed block as a 2-byte short,
    // and if there is still more data to read, you read the next 2-byte header and
    // run uncompress() again.
    /*
     * int bFinal = 1; while (bFinal != 0) { // Read the Final Block flag (1 bit)
     * bFinal = tinf_getbit(); if (bFinal == 0) { return 0; }
     */

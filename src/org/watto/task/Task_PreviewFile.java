  ...
  **/
  ...
  public void redo() {
    ...
    if (SingletonManager.has("ShowMessageBeforeExport")) {
      ...
      else if (currentSidePanel.equals("SidePanel_MeshInvestigator")) {
        //((SidePanel_MeshInvestigator) ComponentRepository.get("SidePanel_MeshInvestigator")).previewFile(path);
        ((WSSidePanelHolder) ComponentRepository.get("SidePanelHolder")).loadPanel("SidePanel_MeshInvestigator");
      }
      else {
        ((SidePanel_Preview) ComponentRepository.get("SidePanel_Preview")).previewFile(path);
      }

...
/**
**********************************************************************************************
When displaying a preview and, in the previewer, the user has edited the file (eg changing the 
value in a tree or a table), this will export the file to the filesystem (if needed)
and then save the changes to that file. Finally, it flags the Archive as being changed.
**********************************************************************************************
**/
public class Task_WriteEditedPreview extends AbstractTask {
  ...
  **********************************************************************************************
  **/
  public Task_WriteEditedPreview(PreviewPanel previewPanel) {
  ...
  **********************************************************************************************
  **/
  public Task_WriteEditedPreview(PreviewPanel previewPanel, Resource resource) {
  ...
  public void redo() {
    ...
    try {
      ...
      if (!SingletonManager.has("CurrentViewer")) {
        // We don't know what the current ViewerPlugin is, for some reason, so we can't write using it
        return;
      }
      ...
      // use the current Viewer plugin to write the file (we checked earlier that it exists in SingletonManager)
      ViewerPlugin viewerPlugin = (ViewerPlugin) SingletonManager.get("CurrentViewer");
      if (viewerPlugin.canEdit(previewPanel)) {
        // if this ViewerPlugin doesn't know how to edit/write this kind of PreviewPanel, then why did we even get here?
        FileManipulator fm = new FileManipulator(exportedPath, true);
        FileManipulator originalFM = new FileManipulator(originalPath,false);
        
        viewerPlugin.edit(originalFM, previewPanel, fm);
      ...
      previewPanel.setObjectChanged(false);

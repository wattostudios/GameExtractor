...
public class WSTabbedPane extends JTabbedPane implements WSComponent {
  ...
  ***********************************************************************************************/
  public WSTabbedPane(XMLNode node) {
    super();
    toComponent(node);
    registerEvents();
  ...
  ***********************************************************************************************/
  ...
  public void toComponent(XMLNode node) {
    ...
    String tag = node.getAttribute("tab-placement");
    if (tag != null) {
      try {
        setTabPlacement(WSHelper.parseTabPlacement(tag));
      }
      catch (Throwable t) {
        try {
          throw new WSComponentException("Invalid tab placement on WSTabbedPane: " + tag);
        }
        catch (Throwable t2) {
          ErrorLogger.log(t2);
        }
    ...
    // add the children as tabs
    int numChildren = node.getChildCount();
    for (int i = 0; i < numChildren; i++) {
      try {
        XMLNode child = node.getChild(i);
        JComponent component = WSHelper.toComponent(child);
        addTab(((WSComponent) component).getCode(), component);
      }
      catch (Throwable t) {
        ErrorLogger.log(t);
      }
  ...
  ***********************************************************************************************/
  ...
  public XMLNode toXML() {
    ...
    node.setAttribute("tab-placement", WSHelper.parseTabPlacement(getTabPlacement()));

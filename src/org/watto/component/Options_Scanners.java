...
public class Options_Scanners extends WSPanelPlugin {
   ...
   **********************************************************************************************
   **/
  public Options_Scanners(XMLNode node) {
    super(node);
  }
  ...
  **/
  public void loadScanners() {
    WSPlugin[] scanners = WSPluginManager.getGroup("Scanner").getPlugins();

    WSPanel checkboxHolder = (WSPanel) ComponentRepository.get("SidePanel_Options_Scanners_CheckBoxHolder");
    checkboxHolder.setLayout(new GridLayout(1, scanners.length));

    java.util.Arrays.sort(scanners);

    checkboxHolder.setLayout(new GridLayout(scanners.length, 1));

    for (int i = 0; i < scanners.length; i++) {
      String scannerCode = scanners[i].getCode();
      checkboxHolder.add(WSHelper.toComponent(XMLReader.read("<WSOptionCheckBox code=\"Option_Scanners_" + scannerCode + "\" setting=\"Scanner_" + scannerCode + "\" />")));

      // Add the texts for these checkboxes, using the names for each scanner.
      // Means that we don'tneed to add them in to the language pack manually.
      String langCode = "WSCheckBox_Scanner_" + scannerCode + "_Text";
      if (!Language.has(langCode)) {
        Language.set(langCode, scanners[i].getName());
      }

      Language.set("WSCheckBox_Scanner_" + scannerCode + "_Tooltip", "Enable or disable this scanner from being used.");
    }
  }
  ...
  public void toComponent(XMLNode node) {
    ...
    XMLNode srcNode = XMLReader.read(new File("interface" + File.separator + "SidePanel_Options_Scanners.xml"));

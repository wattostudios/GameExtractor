...
public final class XXTEA {
  ...
  public static final byte[] decrypt(byte[] data, byte[] key) {
    ...
    int[] dataInt = toIntArray(data, false);
    int[] keyInt = toIntArray(fixKey(key), false);

    int[] decryptedInts = decrypt(dataInt, keyInt);
    byte[] decryptedBytes = toByteArray(decryptedInts, false);

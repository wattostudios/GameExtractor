...
/***********************************************************************************************
A Split Pane GUI <code>Component</code>
***********************************************************************************************/

public class WSSplitPane extends JSplitPane implements WSComponent {
  ...
  ***********************************************************************************************/
  public WSSplitPane(XMLNode node) {
    super();
    toComponent(node);
    registerEvents();
  ...
  ***********************************************************************************************/
  ...
  public void toComponent(XMLNode node) {
    ...
    String tag = node.getAttribute("divider-size");
    if (tag != null) {
      try {
        setDividerSize(Integer.parseInt(tag));
      }
      catch (Throwable t) {
        try {
          throw new WSComponentException("Invalid divider size on WSSplitPane: " + tag);
        }
        catch (Throwable t2) {
          ErrorLogger.log(t2);
        }
    ...
    for (int i = 0; i < numChildren && i < 2; i++) {
      try {
        XMLNode child = node.getChild(i);
        tag = child.getAttribute("position");
        if (tag.equalsIgnoreCase("NORTH") || tag.equalsIgnoreCase("WEST") || tag.equalsIgnoreCase("LEFT") || tag.equalsIgnoreCase("TOP")) {
          setLeftComponent(WSHelper.toComponent(child));
          // Stops the splitpane from being unmovable if one of the components is too large
          leftComponent.setMinimumSize(new Dimension(0, 0));
        }
        else if (tag.equalsIgnoreCase("SOUTH") || tag.equalsIgnoreCase("EAST") || tag.equalsIgnoreCase("RIGHT") || tag.equalsIgnoreCase("BOTTOM")) {
          setRightComponent(WSHelper.toComponent(child));
          // Stops the splitpane from being unmovable if one of the components is too large
          rightComponent.setMinimumSize(new Dimension(0, 0));
        }
        else {
          try {
            throw new WSComponentException("Invalid component position specified on WSSplitPane: " + tag);
          }
          catch (Throwable t) {
            ErrorLogger.log(t);
          }
  ...
  ***********************************************************************************************/
  ...
  public XMLNode toXML() {
    ...
    node.setAttribute("divider-size", "" + getDividerSize());

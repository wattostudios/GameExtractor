...
/**
**********************************************************************************************
Uses FFMPEG to convert an MSADPCM-encoded WAV audio file into a WAV_RIFF audio file, then plays it
**********************************************************************************************
**/
public class Viewer_FFMPEG_Audio_WAV extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_FFMPEG_Audio_WAV() {
    super("FFMPEG_Audio_WAV", "FFMPEG-supported WAV Audio");
    setExtensions("wav", "snd", "audioclip"); // SND/AudioClip so that it picks up WAV files from Unity Engine archives that are in XBox format (IMAADPCM WAV)
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (fm.readShort() == 2) { // MSADPCM Format
        rating += 50;
      }
  ...
  **/
  ...
  public PreviewPanel read(File source) {
    ...
    try {
      ...
      ErrorLogger.log("Viewer_FFMPEG_Audio_WAV could not open the audio file.");
      return null;
    }
    catch (Throwable t) {
      logError(t);

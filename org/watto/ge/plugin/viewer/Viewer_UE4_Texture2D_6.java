...
**********************************************************************************************
**/
public class Viewer_UE4_Texture2D_6 extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_UE4_Texture2D_6() {
    super("UE4_Texture2D_6", "Unreal Engine 4 Version 6 Texture Image");
    setExtensions("texture2d", "uasset"); // MUST BE LOWER CASE

    setGames("Unreal Engine 4 Version 6",
        "BARBAR_BAR",
        "Cresent Bay",
        "THORN");
    setPlatforms("PC");
  ...
  public ImageResource readThumbnail(FileManipulator fm) {
    try {
      ...
      if (type.equals("PF_DXT5") || type.equals("PF_DXT1") || type.equals("PF_DXT3")) {
        ...
        // 4 - null
        if (fm.readInt() == 0) {
          // the offset was actually an 8-byte field
          fm.skip(28);
        }
        else {
          // it was just a 4-byte field
          fm.skip(24);
        }
      ...
      else if (type.equals("PF_FloatRGBA")) {
        // 4 - Offset to Largest Mipmap [+8]
        long largestMipmapOffset = IntConverter.unsign(fm.readInt()) + 8;
        FieldValidator.checkOffset(largestMipmapOffset, arcSize);
        ...
        ImageResource imageResource = ImageFormatReader.read16F16F16F16F_BGRA(fm, width, height);
        imageResource.setProperty("ImageFormat", "16F16F16F16F_BGRA");
        imageResource.setProperty("MipmapCount", "" + mipmapCount);

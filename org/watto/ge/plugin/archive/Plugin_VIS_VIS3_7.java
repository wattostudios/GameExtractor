...
**********************************************************************************************
**/
public class Plugin_VIS_VIS3_7 extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_VIS_VIS3_7() {

    super("VIS_VIS3_7", "VIS_VIS3_7");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("Deponia (Humble Bundle)");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (fm.readByte() == 52 && fm.readByte() == 99 && fm.readByte() == 54 && fm.readByte() == 97) {
        rating += 5;
      }
      else {
        rating = 0;
  ...
  public Resource[] read(File path) {
    try {
      ...
      //49,80,41,52,99,54,97,56
      for (int i = 0; i < dirSize; i += 8) {
        directoryBytes[i] ^= 49;
        directoryBytes[i + 1] ^= 80;
        directoryBytes[i + 2] ^= 41;
        directoryBytes[i + 3] ^= 52;
        directoryBytes[i + 4] ^= 99;
        directoryBytes[i + 5] ^= 54;
        directoryBytes[i + 6] ^= 97;
        directoryBytes[i + 7] ^= 56;
      }

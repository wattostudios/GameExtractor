...
**********************************************************************************************
**/
public class Plugin_DAT_RIDT extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_DAT_RIDT() {

    super("DAT_RIDT", "DAT_RIDT");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (FilenameSplitter.getFilename(fm.getFile()).contains("CompTextures")) {
        rating += 25;
      }
      else {
        rating = 0;
      ...
      File dirFile = new File(fm.getFile().getAbsoluteFile().getParent() + File.separatorChar + "TextureDirectory.dat");
      if (dirFile.exists()) {
        rating += 24;
      }
      else {
  ...
  public Resource[] read(File path) {
    try {
      ...
      File sourcePath = new File(basePath + File.separatorChar + "TextureDirectory.dat");
      ...
      for (int i = 0; i < numFiles; i++) {
        ...
        // 1 - Image Format (3=DXT1, 8=DXT5, 0=empty)
        int imageFormat = fm.readByte();
        if (imageFormat == 0) {
          resource.setLength(0);
          resource.setDecompressedLength(0);
          continue;
        }

...
**********************************************************************************************
**/
public class Plugin_U_154 extends PluginGroup_U {
  ...
  **/
  public Plugin_U_154() {
    super("U_154", "Unreal Engine (Version 154)");

    setExtensions("u", "uax", "ukx", "umx", "upx", "usx", "utx", "uvx");
    setGames("Star Wars: Republic Commando");
    setPlatforms("PC");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    return super.getMatchRating(fm, new int[] { 154, 156, 157, 158 });
  }
  ...
  public Resource[] read(File path) {
    try {
      ...
      for (int i = 0; i < numFiles; i++) {
        ...
        // 4 - File Length                      // CHANGED
        long length = fm.readInt();
        FieldValidator.checkLength(length, arcSize);

        // 4 - File Offset                      // CHANGED
        long offset = 0;
        if (length > 0) {
          offset = fm.readInt();
         ...
         * resources[i].setExporter(Exporter_Custom_U_Palette_154.getInstance()); }
         */
